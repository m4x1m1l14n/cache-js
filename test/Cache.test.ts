...
describe('Cache', () => {
	...
	test('flush - without invoking callbacks', () => {
		const cache = new Cache<number, string>();
		cache.set(1, 'Some string to store in cache');
		cache.set(2, 'Another string to store in cache');
	...
	test('flush - with invoking callbacks', () => {
		const callback = jest.fn();
		const cache = new Cache<number, string>();
		cache.set(1, 'Some string to store in cache', undefined, callback);
		cache.set(2, 'Another string to store in cache', undefined, callback);
